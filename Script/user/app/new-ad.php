...
$datepost = date('M d, Y');
    ...
    $stmt = $conn->prepare("INSERT INTO tbl_ads (ad_id, author, title, category, fuel, ad_condition, price, short_desc, fixed, brand, description, date_posted) VALUES (:adid, :author, :title, :category, :fuel, :adcond, :price, :shortdesc, :fixed, :brand, :description, :datepost)");
    $stmt->bindParam(':adid', $ad_id);
    $stmt->bindParam(':author', $myid);
    $stmt->bindParam(':title', $ad_title);
    $stmt->bindParam(':category', $ad_category);
    $stmt->bindParam(':fuel', $ad_fuel);
